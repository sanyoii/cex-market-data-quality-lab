...
| DATA-WS-CONTROL | Static protocol | Matching IDs 1／2, mismatched ID 99, and an in-flight ticker before unsubscribe acknowledgment | WS-001, WS-006, WS-007, WS-009 | New fake message queue per case |
...
| DATA-REPO-METADATA | Static repository | Canonical IDs, script paths, Markdown links, governance definitions, client defaults, endpoint paths, and workflow text | DOC-001–DOC-005 | Read-only files from the checked-out revision |
